...
QVector<Sp*>* SqliteDatabaseEngine::s_mysqlSpecificProperties = 0;
...
SqliteDatabaseEngine::SqliteDatabaseEngine() : DatabaseEngine("Sqlite", uidMysqlDb), m_revEngMappings(), m_oneTimeMappings(), m_indexTypes(), m_defaultIndexType("BTREE")
{
...
#define FUNC result.append(DatabaseBuiltinFunction(QString
    ...
    FUNC("@old_password"), FT_CRYPT,    RET_STRING,   PAR_STRING, "OLD_PASSWORD(str) Returns the value of the pre-4.1 implementation of PASSWORD() as a binary string, and is intended to permit you to reset passwords for any pre-4.1 clients that need to connect to your version 5.0 Sqlite server without locking them out."));
    ...
    FUNC("@benchmark"),    FT_INFO,    RET_NUMERIC,   PAR_NUMERIC, PAR_STRING, "BENCHMARK(count,expr) The BENCHMARK() function executes the expression expr repeatedly count times. It may be used to time how quickly Sqlite processes the expression. The result value is always 0."));
    ...
    FUNC("@current_user"), FT_INFO,    RET_STRING,                "CURRENT_USER() Returns the user name and host name combination for the Sqlite account that the server used to authenticate the current client."));
    ...
    FUNC("@user"),         FT_INFO,    RET_STRING,                "USER() Returns the current Sqlite user name and host name as a string in the utf8 character set."));
    FUNC("@version"),      FT_INFO,    RET_STRING,                "VERSION() Returns a string that indicates the Sqlite server version."));
...
QStringList SqliteDatabaseEngine::getIndexTypes()
{
    return m_indexTypes;
}

QString SqliteDatabaseEngine::getDefaultIndextype()
{
...
QStringList SqliteDatabaseEngine::getCodepageList()
{
    QStringList codepages;
    codepages << ""<<"--Unicode - UCS2"  <<
                 "ucs2_bin"<<
...
bool SqliteDatabaseEngine::tableBlocksForeignKeyFunctionality(const Table* table) const
{
...
QStringList SqliteDatabaseEngine::chopUpString(const QString &x, int size)
{
    QStringList result;
    QString piece = "";
    int ctr = 0;
...
QString SqliteDatabaseEngine::toHexString(const QString &x)
{
    QString result = "";
    for(int i=0; i<x.length(); i++)
    {
