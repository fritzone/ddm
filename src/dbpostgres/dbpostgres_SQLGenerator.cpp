...
QStringList PostgresSQLGenerator::generateCreateTableSql(Table *table,
                                const QHash<QString, QString> &options,
                                const QString& tabName,
                                const QMap<QString, QString> &fkMappings,
                                const Connection* pdest) const
{
    // error check: Is this a PostgresConnection? (if we have one)
    const PostgresConnection* dest = dynamic_cast<const PostgresConnection*>(pdest);
    if(!dest && pdest)
    {
        return QStringList();
    }
...
QString PostgresSQLGenerator::createTableOnlyScript(Table* table, const QStringList& foreignKeys, const QString& tabName, const Connection *) const
{
    bool needfk = true;
...
QString PostgresSQLGenerator::sqlForAColumn(const Column *col) const
{
